...
│
├── Data/                    # 数据存储层 (5个文件)
│   ├── tile_embeddings.json # 方块向量数据 (179KB)
│   ├── style_embeddings.json # 风格向量数据 (54KB)
│   ├── biome_embeddings.json # 生物群落向量数据 (67KB)
│   ├── terraria_kb.db       # SQLite知识库数据库 (143KB)
│   └── terraria_kb_full.sql # 数据库SQL备份 (29KB)
│
├── Commands/                # 聊天命令层
...
| [description_workshop.txt](description_workshop.txt) | Steam Workshop描述 |
...
| 文件 | 行数 | 功能 | 关键类/方法 |
|------|------|------|-------------|
| [AIAgentService.cs](Core/AIAgentService.cs) | 1238 | AI Agent建筑生成服务，多Agent协作模式 | `GenerateBuildingAsync()`, `GenerateBuildingMultiAgentAsync()`, `PlanBuildingAsync()` |
| [KnowledgeBase.cs](Core/KnowledgeBase.cs) | 602 | 知识库管理器，方块/风格/家具/屋顶/窗户/楼层模板 | `TileKnowledgeBase`, `StyleTemplateBase`, `RoofTemplateBase`, `WindowTemplateBase`, `FloorStructureBase` |
| [ModuleAgents.cs](Core/ModuleAgents.cs) | 619 | 模块生成Agent，并行生成屋顶/墙壁/楼层/窗户/家具 | `GenerateRoofAsync()`, `GenerateWallsAsync()`, `GenerateFurnitureAsync()` |
| [VectorKnowledgeBase.cs](Core/VectorKnowledgeBase.cs) | 504 | 向量知识库，384维embedding语义检索 | `SearchTilesSemantic()`, `CosineSimilarity()` |
| [BuildingDesign.cs](Core/BuildingDesign.cs) | 464 | 建筑设计JSON结构，包含tiles/walls/furniture等 | `BuildingDesign`, `TileData`, `WallData`, `FurnitureData`, `NpcSuitability` |
| [BuildingMerger.cs](Core/BuildingMerger.cs) | 276 | 合并多个模块生成结果，验证并填充缺失元素 | `Merge()`, `ValidateAndFillMissing()`, `DeduplicateTiles()` |
| [BuildingExecutor.cs](Core/BuildingExecutor.cs) | 244 | 基础建筑执行器，解析JSON并放置方块 | `ParseDesign()`, `BuildAtLocation()` |
| [BuildingPlan.cs](Core/BuildingPlan.cs) | 112 | 建筑规划结构，区域划分和参数 | `BuildingPlan`, `Region`, `ModuleResult`, `WindowPosition`, `FurniturePosition` |
| [AIApiService.cs](Core/AIApiService.cs) | 241 | AI API调用封装，支持OpenAI/Claude/DeepSeek格式 | `SendChatRequestAsync()`, `ExtractJsonFromResponse()` |
| [EnhancedBuildingExecutor.cs](Core/EnhancedBuildingExecutor.cs) | 120 | 增强版执行器，支持油漆、斜坡、阴影效果 | `BuildAtLocationEnhanced()` |

### Data 数据存储 (5个文件)

| 文件 | 大小 | 功能 |
|------|------|------|
| tile_embeddings.json | 179KB | 方块向量数据 (384维embedding) |
| style_embeddings.json | 54KB | 风格向量数据 (9种风格) |
| biome_embeddings.json | 67KB | 生物群落向量数据 |
| terraria_kb.db | 143KB | SQLite知识库数据库 |
| terraria_kb_full.sql | 29KB | 数据库SQL备份 |
...
| 文件 | 功能 | 配置项 |
|------|------|--------|
| [AIBuildingConfig.cs](Config/AIBuildingConfig.cs) | 客户端配置类 | `ApiKey`, `ServiceProvider`, `ModelName`, `CustomEndpoint`, `BuildOffsetX/Y`, `MaxBuildingSize` |
...
| 文件 | 功能 | 关键方法 |
|------|------|----------|
| [AIBuildingPlayer.cs](Players/AIBuildingPlayer.cs) | 玩家ModPlayer，管理生成状态和快捷键 | `RequestBuildingDesignAgent()`, `PlaceLastDesign()`, `StopGeneration()`, `GetKnowledgeBaseStatus()` |
...
| 文件 | 功能 | 组件 |
|------|------|------|
| [AIBuildingUI.cs](UI/AIBuildingUI.cs) | AI建筑面板，风格/尺寸/操作/日志 | `AIBuildingPanel`, `AIBuildingUISystem`, `BuildingStyle` |

**UI模块**: 风格选择(6种)、尺寸选择(3种)、操作按钮(生成/放置/选区/Agent)、日志显示(10行)
...
# 可选: pip install sentence-transformers
```
...
| [generate_embeddings_full.py](Tools/vector/generate_embeddings_full.py) | 完整版向量生成 | SentenceTransformers 384维向量 |
...
| 目录 | 文件数 | 内容 |
|------|--------|------|
| tutorial/ | 3 | tModLoader开发教程、编译方式、日志排查 |
| terraria-systems/ | 2 | Tile系统、WorldGen系统技术文档 |
| ai-integration/ | 2 | AI集成方案、Agent架构设计 |
| database/ | 4 | 数据库设计、检索方案、NPC房屋数据 |
| project/ | 2 | 项目结构、模组分析 |
...
├── Tiles (TileKnowledgeBase)     # 方块知识库 (12种基础方块)
├── Styles (StyleTemplateBase)    # 风格模板库 (9种风格)
├── Furniture (FurnitureRuleBase) # 家具规则库 (7种基础家具)
├── Vectors (VectorKnowledgeBase) # 向量检索库 (384维embedding)
├── Roofs (RoofTemplateBase)      # 屋顶模板库 (5种屋顶)
├── Windows (WindowTemplateBase)  # 窗户模板库 (4种窗户)
└── Floors (FloorStructureBase)   # 楼层结构库 (4种结构)
```
